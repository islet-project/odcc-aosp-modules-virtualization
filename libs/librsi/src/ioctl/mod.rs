...
use crate::ioctl::kernel::RsiRealmConfig;
...
/// Reads the measurement from particular index
...
/// Extends the measurement for particular index using provided data
...
/// Requests the Arm CCA attestation token using provided challenge
pub fn attestation_token(challenge: &[u8; super::CHALLENGE_LEN as usize]) -> nix::Result<Vec<u8>>
{
    ...
    let mut token = vec![0u8; INITIAL_TOKEN_SIZE as usize];
    ...
    match kernel::attestation_token(fd.get(), &mut attest) {
        Ok(_) => (),
        Err(Errno::ERANGE) => {
            token = vec![0u8; attest[0].token_len as usize];
            attest[0].token = token.as_mut_ptr();
...
/// Retrieves the Realm Config
pub fn realm_config() -> nix::Result<RsiRealmConfig>
{
    let mut realm_config = [kernel::RsiRealmConfig::default()];
    let fd = Fd::wrap(nix::fcntl::open(DEV, FLAGS, MODE)?);
    kernel::realm_config(fd.get(), &mut realm_config)?;
    Ok(realm_config[0].clone())
}
