...
fn unpack_token_realm(token: &mut RealmToken) -> Result<(), TokenError>
{
    ...
    for (i, rem) in rems.into_iter().enumerate() {
        let claim = token.measurement_claims.get_mut(&(i as u32)).unwrap();
        get_claim(rem, claim)?;
    }
...
fn verify_token_realm(buf: &[u8]) -> Result<RealmToken, TokenError>
{
    ...
    let realm_public_key = token.token_claims[&CCA_REALM_PUB_KEY].data.get_bstr();
    crypto::verify_coset_signature(&token.cose_sign1, realm_public_key, b"")?;

    Ok(token)
}

/// Verifies the Arm CCA platform token
...
/// Verifies the Arm CCA token (both the realm and platform parts)
