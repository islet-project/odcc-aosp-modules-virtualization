//! Implementation of white-list policy of TCP/IP servers
...
{
    /// Creates an empty runtime statistics structure
    pub fn new() -> Self
    {
        ServerStatsHashMap {
            stats: HashMap::new(),
...
impl PolicyManager
{
    /// Creates an empty instance of PolicyManager
    pub fn new() -> Self
    {
        PolicyManager {
    ...
    fn init(&self, whitelist: &ServerWhitelist) -> io::Result<()>
    {
        ...
        *guard = whitelist.clone();

        Ok(())
    }

    /// Loads the policy from JSON file
    pub fn load_from_file(&self, filename: &str) -> io::Result<()>
    {
    ...
    /// Loads the policy from reader
    pub fn load_from_reader<R>(&self, rdr: R) -> io::Result<()>
    where
        R: std::io::Read,
    {
        let whitelist = serde_json::from_reader(rdr)?;
        self.init(&whitelist)?;
        Ok(())
    }
    ...
    {
        let whitelist_guard = self.whitelist.read().expect("Failed to acquire read lock");
        if whitelist_guard.is_empty() {
            info!("The policy file is empty!");
            return;
        }
