//! Helper functions for copying streams
...
) -> io::Result<()>
{
    ...
    let vsock_read_handle = thread::spawn(move || -> io::Result<()> {
        ...
        // VSOCK <- TCP stream handling thread to close sockets. It shuts down connections immediately
        // preventing half-open connections.
        ...
        if let Err(e) = vsock_read.shutdown(vsock_read_shutdown) {
            info!("Vsock read shutdown (expected on peer close): {}", e);
        }
        if let Err(e) = tcp_write.shutdown(tcp_write_shutdown) {
            info!("TCP write shutdown (expected on peer close): {}", e);
        }
        result
    });
